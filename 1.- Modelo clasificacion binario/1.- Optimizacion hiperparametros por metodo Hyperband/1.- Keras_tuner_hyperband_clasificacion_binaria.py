#-------------------------------------------------------------------------------
# Name:        Optimización para el modelo clasificación binaria de imágenes
#              sobre tumores cerebrales
# Purpose:     Se compone del proceso de optimización mediante "Hyperband",
#              de la arquitectura para clasificación binaria de los tumores
#              cerebrales.
#
...
#   Arquitectura clasificacion binaria de tumores cerebrales.
    model = Sequential()
    ...
    model.add(Dropout(hp.Choice('Dropout_1', values=[0.25, 0.50])))
    ...
    model.add(MaxPooling2D(pool_size=(2,2), strides=(2, 2)))
    model.add(Dropout(hp.Choice('Dropout_2', values=[0.25, 0.50])))

    model.add(Conv2D( filters     = hp.Choice('conv_3_filters', values=[128, 512]),
    ...
    model.add(MaxPooling2D(pool_size=(2,2), strides=(2, 2)))
    model.add(BatchNormalization())
    model.add(Dropout(hp.Choice('Dropout_3', values=[0.25, 0.50])))

    model.add(Conv2D( filters     = hp.Choice('conv_4_filters', values=[128, 512]),
                      kernel_size = hp.Choice('conv_4_kernel', values=[3, 5]),
                      padding     ='same',
                      activation  = activations_map[hp.Choice('conv_4_activation', values=['relu','leaky'])]))

    model.add(MaxPooling2D(pool_size=(2,2), strides=(2, 2)))
    model.add(Dropout(hp.Choice('Dropout_4', values=[0.25, 0.50])))

    model.add(Conv2D( filters     = hp.Choice('conv_5_filters', values=[128, 512]),
                      kernel_size = hp.Choice('conv_5_kernel', values=[3, 5]),
                      padding     = 'same',
                      activation  = activations_map[hp.Choice('conv_5_activation', values=['relu','leaky'])]))

    model.add(BatchNormalization())
    model.add(Dropout(hp.Choice('Dropout_5', values=[0.25, 0.50])))

    model.add(Flatten())
    model.add(Dense(units=hp.Int('dense_1_units', min_value=32, max_value=256, step=32), activation= 'relu'))
    model.add(Dropout(hp.Choice('Droput_6', values=[0.25, 0.50])))

    model.add(Dense(units=hp.Int('dense_2_units', min_value=32, max_value=256, step=32), activation= 'relu'))
...
def explore_hypermodel(i_dataset_train, i_dataset_test):
    ...
    tuner = kt.Hyperband(
            ...
            overwrite    = True,
            directory    = "D:\logs",
            project_name = "Binary_Turner",
    )

    # Ejecutar la búsqueda de hiperparámetros
    tuner.search(i_dataset_train, epochs=5, validation_data=i_dataset_test)
   ...
   # Visualizar summary
    ...
    with open('results_summary_clasificacion_binaria.txt', 'w') as f1:
        with redirect_stdout(f1):
            tuner.results_summary()

    with open('best_model_summary_clasificacion_binaria.txt', 'w') as f2:
...
#-----------------------------------------------------------------------------
# Ejecutamos todo el proceso de optimización
#-----------------------------------------------------------------------------
def main():

    # Directorios de trabajo
    l_directory_train = 'D:\\Copia 22-09-2025\\TFG\\2.- Modelo clasificacion binario\\1.- Optimizacion hiperparametros por metodo Hyperband\\1.- Datos optimización tumor y no tumor\\1.- Entrenamiento'
    l_directory_test  = 'D:\\Copia 22-09-2025\\TFG\\2.- Modelo clasificacion binario\\1.- Optimizacion hiperparametros por metodo Hyperband\\1.- Datos optimización tumor y no tumor\\2.- Validacion'
