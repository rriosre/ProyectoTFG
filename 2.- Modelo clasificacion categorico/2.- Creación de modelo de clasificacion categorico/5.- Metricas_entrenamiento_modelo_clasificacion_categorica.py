#-------------------------------------------------------------------------------
# Name:        Métricas - Modelo clasificación categórica
...
def main():
    ...
    history = np.load('history_clasificacion_categorica.npy', allow_pickle=True).item()
    ...
    for i in range(num_epocas):
        ...
        loss  = h.get('loss', [0])[i]
        acc   = h.get('accuracy', h.get('acc', [0]))[i]
